...
Korrekturen 2026-09-17 (Befund aus Lauf #11 und #12: 0 Fehler, Abdeckung
100 %, aber Exit 1 und nichts committet):

  7. Das Collapse-Tor verglich 6 neu qualifizierte Wallets gegen 14 aus der
     VORGAENGERMESSUNG. Die alte Zahl stammt aus einer anderen Methode und ist
     mit Version 4 per Konstruktion nicht reproduzierbar -- die Sperre konnte
     sich also nie mehr loesen und haette jeden weiteren Lauf identisch
     blockiert. Das Retention-Kriterium greift jetzt nur noch gegen einen
     Vorlauf derselben measurement_version; ueber einen Versionswechsel hinweg
     bleibt allein MIN_ACTIVE_ABS stehen. Ein Einbruch INNERHALB einer Version
     stoppt das Update unveraendert.
  8. run() gab fuer jeden Nicht-Publish Exit 1 zurueck. Damit schlug der Schritt
     fehl, der Workflow brach ab und der Commit-Schritt lief nie -- ausgerechnet
     der Vorschlag und die Diagnose, fuer die das Tor gebaut wurde, gingen
     verloren. Jetzt: 0 ersetzt, 1 Befund mit gueltigem Vorschlag, 2
     Fehlfunktion. Der Workflow committet in allen drei Faellen und wird nur
     bei 2 rot.
...
MIN_ACTIVE_ABS = 3
# Die Retention-Haelfte dieses Tors vergleicht zwei Laeufe DERSELBEN Messung.
# Wechselt die Messversion, gibt es keinen vergleichbaren Vorlauf mehr -- dann
# greift nur die absolute Untergrenze. Siehe Korrektur 7 im Modulkopf.
...
def build_watchlist(results, previous):
    ...
    return {'schema_version': 1, 'measurement_version': MEASUREMENT_VERSION, 'active': active[:MAX_ACTIVE],
...
def run(args, client=None):
    ...
    prev_version = previous.get('measurement_version')
    comparable = prev_version == MEASUREMENT_VERSION
    # Ein Einbruch der qualifizierten Wallets ist ein Befund, kein Update --
    # aber nur gegen einen Vorlauf DERSELBEN Messung. Ueber einen
    # Versionswechsel hinweg ist die alte Zahl per Konstruktion nicht
    # reproduzierbar; der Vergleich sperrt sich dann selbst dauerhaft ein
    # (Lauf #11 und #12: 6 aktiv gegen 14 aus der Vorgaengermessung).
    active_floor = (max(MIN_ACTIVE_ABS, MIN_ACTIVE_RETENTION*prev_active)
                    if comparable else MIN_ACTIVE_ABS)
    no_collapse = prev_active == 0 or qualified >= active_floor
    ...
    for r in results:
        for key, count in (r.get('skipped') or {}).items():
            reasons[key] += count
    report = {'schema_version': 1, 'measurement_version': MEASUREMENT_VERSION,
              ...
              'no_collapse': no_collapse, 'active_floor': active_floor,
              'previous_measurement_version': prev_version,
    ...
    if publish and not args.dry_run:
        ...
        print(f'Watchlist NICHT ersetzt. Abdeckung {coverage:.0%} (Grenze '
              ...
              f'{qualified} gegen vorher {prev_active} (Grenze {active_floor:.1f}, '
              f'no_collapse={no_collapse}, Vorlauf-Messversion {prev_version}, '
              f'vergleichbar={comparable}), '
    ...
    # Exit-Code trennt Befund von Fehlfunktion, damit der Workflow beides
    # unterschiedlich behandeln kann:
    #   0  Watchlist ersetzt.
    #   1  nicht ersetzt, aber ein gueltiger Vorschlag liegt vor (Befund).
    #      Der Workflow committet ihn und meldet eine Warnung.
    #   2  Fehlfunktion: Lauf abgebrochen oder Bestand nicht sauber gemessen.
    if publish:
        return 0
    if (run_error or not results or failure_rate > MAX_FAILURE_RATE
            or coverage < MIN_ROSTER_COVERAGE):
        return 2
    return 1
...
def main():
    ...
    try:
        ...
        # Abbruch ist eine Fehlfunktion, kein Befund: Exit 2, damit der
        # Workflow rot wird und nicht bloss eine Warnung schreibt.
